...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (mysqli_num_rows($result_email) == 0) {
        ...
        <html>
        <body>
        <p>Hier ist ihr Ruecknahmeprotokoll :)</p>
        </body>
        </html>'; 
        $pdf = new FPDF();
        $pdf->AddPage();
        $pdf->SetFont('Arial','B',16);
        $pdf->Cell(40,10,'Ruecknahmeprotokoll:');
        $pdf->Cell(40,10,'Hier steht Inhalt:');
        $doc = $pdf->Output('S');
        send_mail($recipient, $subject, $message,$doc,"Ruecknahmeprotokoll");
        die;
    }
}
?>
